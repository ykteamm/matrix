          ...
          <div class="justify-content-between align-items-center p-2" >
            ...
            <div class="btn-group mr-5 ml-auto">
                <div class="row">
                   ...
                   <div class="col-md-12">
                      <button type="button" id="action_user" name="{{$action}}" class="btn btn-block btn-outline-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> {{$factor[$action]}} </button>
                      <div class="dropdown-menu timeclass">
                          @foreach ($factor as $key => $item)
                              <a type="button" onclick="factor(`{{$key}}`,`{{$item}}`)" class="dropdown-item">{{$item}}</a>
                          @endforeach
                      </div>
                   </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-nowrap mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>#{{$key+1}}</td>
                                <td>{{$users[count($users)-$key-1]['name']}}</td>
                                <td>{{number_format($users[count($users)-$key-1]['allprice'],0,'','.')}}</td>
                            </tr>
                            @endforeach
                            @else
                            @foreach ($users as $key => $item)
...
@section('admin_script')
    <script>
        ...
        {
            $('#action_user').text(name);
            $('#action_user').attr('name',id);
        }
        function search()
        {
            var region = $('#reg_user').attr('name');
            var time = $('#time_user').attr('name');
            var action = $('#action_user').attr('name');

            var url = "{{ route('rm-region',['region' => ':region','time' => ':time','action' => ':action']) }}";
