            ...
            <table class="table table-striped">
                ...
                @foreach($medicines as $medicine)
                    <tr>
                        ...
                        <td>
                            <input class="soni{{$medicine->id}}" onkeyup="inputPrice(this)" type="number" name="numbers.{{$medicine->id}}" value="0">
                        </td>
                        @foreach ($price as $key => $item)
                        <td class="narx{{$key}} narxcal{{$key}}{{$medicine->id}} allnarx @if($key != 3) d-none @endif">
...
{{--            <button type="submit" href="" class="btn btn-primary my-2 w-100">Saqlash</button>--}}
    ...
    <div class="text-right " style="position: fixed;right: 1rem;top: 62px; width: 100%"  >
        <div class="row">
            ...
            <div class="col-md-6">
                <button type="button" id="forprice" style="width: 83.5%;" class="btn btn-primary">Plan summasi </button>
            </div>
        </div>
    </div>
</form>
    ...
    <script>
        function priceChange(id)
        {
            ...
            $.each(medId, function( index, value ) {
                sum = sum + (parseInt($(`.narxcal${id.value}${value.id}`).html()) * parseInt($(`.soni${value.id}`).val()));
            });
