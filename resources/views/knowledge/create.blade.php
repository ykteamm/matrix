                            ...
                            <form action="{{ route('knowledge.store') }}" method="POST">
                                ...
                                <div class="table-responsive">
                                    <table class="table mb-0" id="dtBasicExample">
                                        ...
                                        <tbody>
                                            @isset($knowledges)
                                            @foreach($knowledges as $key => $question)
                                            <tr>
                                                <td> {{ $key+1 }} </td>
                                                <td> {{ $question->name }} </td>
                                                <td class="text-right">
                                                  ...
                                                  {{-- @isset(Session::get('per')['rol_update']) --}}
                                                    <a href="{{ route('knowledge.edit',$question->id) }}" class="btn btn-sm btn-white text-success mr-2"><i class="fas fa-edit mr-1"></i></a>
                                                  {{-- @endisset --}}
                                                  {{-- @isset(Session::get('per')['rol_delete']) --}}
                                                    {{-- <a href="{{ route('knowledge.delete',$question->id)}}" class="btn btn-sm btn-white text-danger mr-2"><i class="far fa-trash-alt mr-1"></i></a> --}}
                                                  {{-- @endisset --}}
                                                 </td>
