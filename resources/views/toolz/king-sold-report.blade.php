                     ...
                     <div class="col-md-12" style="">
                        ...
                        <div class="dropdown-menu" style="left:150px !important;overflow-y:scroll; height:400px;">
                           ...
                           @foreach($users as $keyd => $p)
                            @if($regkey == 'all')
                            <a href="#" onclick="pharm(`{{$p->last_name.''.$p->first_name}}`,`{{$p->id}}`)" class="dropdown-item pharm{{$p->region_id}} allpharm"> {{$p->last_name}} {{$p->first_name}} </a>
                            @else
                            <?php $pa =  $p->region_id?>
                                @if($regkey == $p->region_id)
                                <a href="#" onclick="pharm(`{{$p->last_name.''.$p->first_name}}`,`{{$p->id}}`)" class="dropdown-item pharm{{$p->region_id}} allpharm"> {{$p->last_name}} {{$p->first_name}} </a>
                                @else 
                                <a style="display:none;" href="#" onclick="pharm(`{{$p->last_name.''.$p->first_name}}`,`{{$p->id}}`)" class="dropdown-item pharm{{$p->region_id}} allpharm"> {{$p->last_name}} {{$p->first_name}} </a>
                                @endif
                            @endif
                           @endforeach
                        </div>
