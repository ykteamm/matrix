...
class NewOrder extends Component {
    ...
    public function mount() {
        ...
        $this->shablons = Shablon::orderBy('name','ASC')->get();
    }
    ...
    public function selectType($value) {
        ...
        if($this->outer == 2)
        {
            $this->pharmacy_or_user = User::whereIn('status',[0,1])->where('region_id',$region_id)->get();
        }
    ...
    public function addProd($id) {
        if(!in_array($id,$this->prod_array))
        {
            ...
            $pr = Medicine::with(['price' => function($q){
                $shablon_id = Shablon::where('id',$this->shablon_id_db)->first();
    ...
    public function saveOrderPharmacy()
    {
        ...
        $new_order = NewOrderModel::create([
